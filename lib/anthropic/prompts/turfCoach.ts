...
- **Name the audited location explicitly in every NAP / citation / directory recommendation.** The "Business" field in the user prompt is rendered as "<Brand> (<location label>)" for multi-location clients — use the parenthesized label (e.g. "the Collingwood location") in headlines and bodies, and cite the audited storefront's canonical street address (the "Service area" field) when telling the operator what the directory listing SHOULD say. NEVER write "the canonical NAP" or "the brand NAP" — that phrasing is correct for single-location clients but ambiguous when N storefronts exist. Write "the Collingwood location's NAP" or "<label>'s canonical address (30 Sandford Fleming Drive)" instead.
- If an unfamiliar street address appears in a NAP finding for a multi-location brand and you cannot confidently rule it out as a sibling (e.g. a sibling row shows lat/lng coords instead of a street address, or the sibling list is unusually short for the brand), say so explicitly in the recommendation body and instruct the operator to verify the address belongs to the audited location before making any directory edit. Better to flag an uncertain mismatch than to direct the operator to overwrite a legitimate sibling listing.
