...
export default async function ScanLandingPage({
  ...
  return (
    <div className="min-h-screen w-full text-white">
      ...
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* 02.5 — Client testimonial                                       */}
      {/* ═══════════════════════════════════════════════════════════════ */}
      {/* Single anonymized testimonial, placed near the top — right
       *  after the buyer sees the sample heatmap. Sequence becomes:
       *  hero (claim) → visual proof (here's what it produces) →
       *  testimonial (here's a client who used the output) → then
       *  the longer explainer (problem / what you get / etc). No
       *  section header — the quote stands on its own; labeling it
       *  "Testimonials" makes it read as a marketing wall instead of
      ...
      >
        <div className="max-w-2xl mx-auto">
          ...
          {/* "Also includes" line — surfaces the secondary
           *  deliverable buyers don't realize they get with the scan.
           *  90-day persistent share URL is real (scan_share_links
           *  TTL); Pulse trial is offered at /order/success. The old
           *  "branded PDF" claim was removed — no scan-tier PDF is
           *  generated by the fulfill route (only audit/strategy
           *  tiers produce the 90-Day Roadmap PDF). */}
          <p className="text-sm text-zinc-500 leading-relaxed mb-8 italic">
            Also includes a 90-day shareable dashboard link and an
            optional 30-day Pulse trial offered at checkout.
          </p>
