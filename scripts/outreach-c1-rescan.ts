...
const INPUT_CSV = path.join(OUTREACH_ROOT, '01-lists/c1/c1-rescan-queue.csv');
...
const PROGRESS_PATH = path.join(RESULTS_DIR, '_rescan-progress.csv');
...
async function main() {
  ...
  if (capIdx !== -1) {
    ...
    console.log(`[c1-rescan] live DFS balance = ${balance !== null ? '$' + balance.toFixed(2) : 'unknown'} -> effective cap = $${cap.toFixed(2)}`);
  }
  ...
  console.log(`[c1-rescan] queue=${work.length} resuming with cumulative=$${cumulative.toFixed(3)} from ${progress.size} prior progress rows`);
  ...
  console.log(`\n[c1-rescan] DONE. scanned=${scanned} failed=${failed} skipped(already-done-this-pass)=${skippedAlready} remaining=${work.length - scanned - failed - skippedAlready} cumulative=$${cumulative.toFixed(3)} / cap $${cap.toFixed(2)}`);
  console.log(`[c1-rescan] ranked-at-pin=${rankedAtPin} absent-at-pin=${absentAtPin} (this run only; see rollup for full history)`);
  if (stopReason) console.log(`[c1-rescan] STOPPED EARLY: ${stopReason}`);
}

main().catch((e) => {
  console.error('[c1-rescan] fatal:', e instanceof Error ? e.stack ?? e.message : e);
  process.exit(1);
});
